...
public class AddCoinsCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!player.hasPermission("enderdragonattack.addcoins")) {
            player.sendMessage(ChatColor.RED + "You do not have permission to use this command.");
            return true;
        }

        if (args.length < 2) {
            player.sendMessage(ChatColor.RED + "Usage: /addcoins <player> <amount>");
            return true;
        }
        ...
        playerDataBase.addPlayerCoins(target, amount);

        player.sendMessage(ChatColor.GREEN + "Added " + amount + " coins to player " + target.getName());
